...
var todoItem = {
    props: ['todo'],
    template: '<li class="bg-white px-4" :id="todo.id" @mouseover="editar=true" @mouseleave="editar=false">\
                ...
                <div style="display:inline-flex!important" class="flex items-center border-b border-grey-lighter">\
                    <div class="flex-grow flex items-center py-4">\
                        <input type="checkbox" class="mr-3">{{ todo.text }}</div>\
                </div>\
                <i style="float:right;cursor:pointer" v-show="editar" class="fa fa-edit"></i>\
            </li>',
    data:function(){
        return {
            editar:false
...
var todo = {
    template:'<ul class="list-reset dropzone" ref="listaCompleta">\
...
var comp1 = {
    template:"<todo :list='groceryList'>\
    </todo>",
    components: {
        todo
    },
